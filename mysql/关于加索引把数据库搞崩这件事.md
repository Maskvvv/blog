...
所以这里就有一个非常值得注意的地方，当 DDL 一个 DML 非常频繁热表时，如果该 DDL 操作被阻塞，那么后面的 DML 操作都会被阻塞，数据库线程可能会被瞬间打满，导致数据库不可用，这是非常危险的。
...
1. COPY：MySQL 5.6 之前使用的算法，如果采用这种算法的 DDL 期间会阻塞后续的所有 DML 和 DDL，**也就是 这种算法不 ”Online“**
2. INPLACE：MySQL 5.6 出现的新算法，**整体上可以看作是 ”Online“ 的**
...
因为当真正进行耗时的 DDL 操作时，降级成了读锁，其他线程可以对该表进行 DML，大大降低了 DDL 对该表的影响，提高该表的并发，只有在读写锁升降级的时候才会阻塞 DML，所以说 **INPLACE 整体上是 ”Online“**。

## 可是我怎么知道哪种 DDL 支持哪种算法，并且是否是 ”Online“ 的呢？

其实 [MySQL 的官方](https://dev.mysql.com/doc/refman/8.0/en/innodb-online-ddl-operations.html)给出所有 DDL 支持哪种算法,是否是 ”Online“ 的说明，以**添加非主键索**引为例，这里可以看到，他是不支持 INSTANT 算法，但是支持 INPLACE 算法，并且他是 ”Online“ 的。
...
| :---------------------------------------: | :------------------------------------------------------: | :-----------------------------------------------: | ---- |
| begin;<br />select * from table1 limit 1; |                                                          |                                                   | T1   |
|                                           | alter table1 add column1 int, ALGORITHM=INPLACE;（阻塞） |                                                   | T2   |
|                                           |                                                          | begin;<br />select * from table1 limit 1;（阻塞） | T3   |
|                  commit;                  |                                                          |                                                   | T4   |
|                                           |                                                          |                       执行                        | T5   |
|                                           |                           阻塞                           |                                                   | T6   |
...
思考一下如果在 session B 被阻塞（T6 ）的时候，来了个 session D 他想要执行了一次 DML，会发生什么？答案是，他也会被阻塞，具体原因相信聪明的你通过上面的内容肯定知道答案了。
...
> 缩小 VARCHAR 列的大小的 DDL，支持 COPY 算法。
...
这时候 kill 可能未必管用，因为新的请求马上就来了。当然你可以在业务低峰期进行 DDL，但是比较理想的机制是，**在 alter table 语句里面设定等待时间**，如果在这个指定的等待时间里面能够拿到 MDL 写锁最好，拿不到也不要阻塞后面的业务语句，先放弃。之后开发人员或者 DBA 再通过重试命令重复这个过程。
