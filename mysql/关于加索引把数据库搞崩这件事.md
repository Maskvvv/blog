...
DDL 操作，在执行时，不管何种算法，都会经历三个阶段：准备阶段（prepare）、执行阶段（DDL）、提交阶段（commit），Online DDL 的详细加锁过程比较复杂，这里不考虑 DDL `ALGORITHM` 、`LOCK` 等参数的选择，只使用默认的方式，可以将 Online DDL 的过程简单概况为一下这 5 步：
...
因为当真正进行耗时的 DDL 操作，降级成了读锁，其他线程可以对该表进行 DML，大大降低了 DDL 对该表访问的影响，提高了性能。
